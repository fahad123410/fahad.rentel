...
class Config:
    ...
    # Fallback: look for a local serviceAccountKey.json in the same folder as app.py
    _FALLBACK_FIREBASE_SA_PATH = os.path.join(BASE_DIR, "serviceAccountKey.json")
    _EFFECTIVE_FIREBASE_SA_PATH = (
        _DEFAULT_FIREBASE_SA_PATH
        if os.path.exists(_DEFAULT_FIREBASE_SA_PATH)
        else (_FALLBACK_FIREBASE_SA_PATH if os.path.exists(_FALLBACK_FIREBASE_SA_PATH) else "")
    )
